...
class TestPagination:
    ...
    def test_same_domain_duplicate_skipped_within_source(self, taaft_config) -> None:
        """Alpha Writer (page 1) and Alpha Writer Pro (page 2) share a domain."""
        source = make_source(
            {LISTING_URL: PAGE1, f"{LISTING_URL}?page=2": PAGE2}, taaft_config
        )
        names = {c.name for c in source.discover(max_pages=2)}
        assert "Alpha Writer" in names
        assert "Alpha Writer Pro" not in names
