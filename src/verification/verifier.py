...
#: Dead-site phrases that are also perfectly normal *feature labels* on a live
#: product page ("Batch Processing — Coming Soon", a "Sunset" image preset).
#: Buried in body copy they prove nothing, so they only count as a dead-site
#: claim when the page makes them page-level (``<title>``/``h1``/``h2``).
#: This tightens the evidence requirement; it never grants a status.
AMBIGUOUS_DEAD_MARKERS = frozenset(
    {"coming soon", "under construction", "sunset", "no longer available"}
)
...
class OfficialPageEvidence:
    ...
    #: Ambiguous dead-site phrases seen only in body copy (context, not a failure).
    ambiguous_dead_mentions: list[str] = field(default_factory=list)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "ambiguous_dead_mentions": list(self.ambiguous_dead_mentions),
...
def extract_official_evidence(
    ...
    *,
    ...
    # Page-level slots only, so an ambiguous phrase must be the page's own
    # headline claim rather than one feature label inside a long marketing page.
    prominent = " | ".join(
        part.casefold()
        for part in (evidence.title, *evidence.headings)
        if part
    )
    for marker in DEAD_SITE_MARKERS:
        if marker not in lowered_text:
            continue
        if marker in AMBIGUOUS_DEAD_MARKERS and marker not in prominent:
            # Recorded as context, never as a dead-site failure.
            evidence.ambiguous_dead_mentions.append(marker)
            continue
...
def _name_matches(product_name: str, value: str) -> bool:
    ...
    Two properties matter here, and both were learned from live pages:

    * the haystack is *free text* (a page title, heading or ``alt``), so it is
      canonicalized **without** tagline trimming — ``"… Solver Online |
      Tenorshare AI Math"`` brands itself after the separator;
    * noise-token handling must be *symmetric*. The needle drops noise words
      (``ai``, ``free``, ``online``…), so the haystack must be compared in both
      forms: kept-noise (exact substring) and dropped-noise (so
      ``"Clever AI Detector"`` still matches a title that spells the brand out
      verbatim). Only a genuine containment hit counts — nothing is loosened
      beyond removing the same tokens from both sides.
    """
    ...
    haystacks = {
        canonical_name(value, drop_noise=False, drop_tagline=False),
        canonical_name(value, drop_noise=True, drop_tagline=False),
    }
    haystacks.discard(None)
    if not haystacks:
        return False
...
def _evidence_notes(evidence: OfficialPageEvidence) -> list[str]:
    ...
    for code in evidence.failures:
        ...
        notes.append(note)
    if evidence.ambiguous_dead_mentions:
        notes.append(
            "body copy mentions '"
            + "', '".join(evidence.ambiguous_dead_mentions)
            + "' but not as a page-level claim — not treated as a dead site"
        )
