...
#: Link paths whose values rotate per response and therefore carry no identity
#: (Cloudflare's email-obfuscation links re-key on every render).
_FINGERPRINT_NOISE_PATHS = ("/cdn-cgi/",)


def fingerprint_links(markup: str | None, *, limit: int = 400) -> list[str]:
    """Identity-bearing links of a page, in document order.

    Only ``scheme://host/path`` is kept. Three classes of noise are removed
    because they made the paginator's repeat guard unreliable on live pages:

    * the **query string** — a directory that echoes page 1 for ``?page=2``
      still renders its own paginator links with ``?page=2`` appended, which
      used to make an identical page look different (verified on
      ``creati.ai/ai-tools/categories/<slug>/``);
    * per-response **rotating links** (``/cdn-cgi/l/email-protection#…``);
    * **repeats** — a card linked twice contributes once.
    """
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        try:
            ...
            continue
        if any(hint in parts.path for hint in _FINGERPRINT_NOISE_PATHS):
            continue
        key = (
...
def page_fingerprint(markup: str | None) -> str | None:
    """Stable hash of a page's *content identity*.

    Used by the paginator to detect a site that silently serves page 1 again
    for ``?page=2`` (both Creati.ai listing layouts do exactly this).
    """
    if not markup:
        return None
    soup = parse_html(markup)
    if soup is None:
        return None
    links = fingerprint_links(markup)
    basis = "\n".join(links) or (soup.get_text(" ", strip=True)[:4000])
