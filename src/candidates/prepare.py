...
class CandidatePreparer:
    ...
    def prepare_batch(
        ...
        """Prepare a batch, returning ``(pairs, rejected, report)``.

        Each accepted candidate is returned *paired* with its prepared record,
        so a caller can keep working with the original candidate without
        re-deriving its identity (which would risk the two drifting apart).
        """
        report = PreparationReport(started_at=_iso(self._now or _utcnow()))
        pairs: list[tuple[CandidateTool, PreparedCandidate]] = []
        ...
        return pairs, rejected, report

    def prepare_many(
        self, candidates: Iterable[CandidateTool]
    ) -> tuple[list[PreparedCandidate], list[RejectedCandidate], PreparationReport]:
        """Prepare a batch, collecting an auditable report."""
        pairs, rejected, report = self.prepare_batch(candidates)
        return [prepared for _, prepared in pairs], rejected, report
